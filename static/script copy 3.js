...
// === CRM Tab Functionality ===

// Handle tab switching for CRM
document.getElementById('tab-crm').onclick = function() {
  document.getElementById('tab-inbox').classList.remove('active');
  document.getElementById('tab-reply').classList.remove('active');
  document.getElementById('tab-crm').classList.add('active');
  document.getElementById('tab-inbox-content').style.display = 'none';
  document.getElementById('tab-reply-content').style.display = 'none';
  document.getElementById('tab-crm-content').style.display = 'block';
  fetchAccounts('');
};

document.getElementById('crm-search-btn').onclick = function() {
    ...
    .then(accounts => {
      const html = accounts.length
        ? accounts.map(a => `<div>
            ...
            Child: ${a.child_name || '-'}<br>
            Stage: ${a.admissions_stage || '-'}<br>
            <button onclick="viewAccount(${a.id})">View</button>
          </div><hr>`).join('')
        : 'No accounts found.';
      document.getElementById('crm-results').innerHTML = html;
    });
}

// Add new account
document.getElementById('crm-add-form').onsubmit = function(e) {
  e.preventDefault();
  const form = e.target;
  const data = {
    ...
    admissions_stage: form.admissions_stage.value,
...
window.viewAccount = function(id) {
  fetch('/api/accounts/' + id)
    ...
    .then(a => {
      ...
      document.getElementById('crm-account-details').innerHTML =
        ...
        Stage: ${a.admissions_stage || '-'}<br>
        Notes: ${a.notes || '-'}<br>
        Created: ${a.created_at}<br>
        Updated: ${a.updated_at}<br>
        `;
    });
};
